...
(when (= (utils/get-env "NODE_ENV") "production")
  (.use app (.static express (.join path dirname "/public")))
  (.use app (.static express (.join path  dirname "/frontend/dist")))
  (.get app "*" (fn [req res]
                  (->
                   ^js res
                   (.sendFile ^js (.resolve ^js path dirname "frontend" "dist" "index.html"))))))
